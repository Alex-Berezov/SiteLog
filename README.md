...
3. Откройте приложение в браузере:
   ...
   - Backend API: [http://localhost:3001/api/work-logs](http://localhost:3001/api/work-logs)
...
```
Убедитесь, что строка подключения в `server/.env` корректна:
```env
DATABASE_URL="postgresql://postgres:postgres@localhost:5432/sitelog"
```
...
yarn dev
```
Сервер запустится на [http://localhost:3001](http://localhost:3001).
...
yarn dev
```
Клиент запустится на [http://localhost:3000](http://localhost:3000). Запросы к API автоматически проксируются на бэкенд благодаря настройкам Vite.

...
- [ ] **3.1 Справочник видов работ**
  - [ ] `GET /api/work-types` — список всех видов работ
  - [ ] Роут → контроллер → сервис
  - [ ] Обработка ошибок
- [ ] **3.2 CRUD для журнала работ**
  - [ ] Zod-схемы валидации для create/update
  - [ ] `GET /api/work-logs` — список записей с фильтрацией и сортировкой
  - [ ] `POST /api/work-logs` — создание записи
  - [ ] `PUT /api/work-logs/:id` — обновление записи
  - [ ] `DELETE /api/work-logs/:id` — удаление записи
  - [ ] Middleware для валидации запросов через zod
  - [ ] Централизованный error handler
- [ ] **3.3 Тесты бэкенда**
  - [ ] Настроить Jest + Supertest
  - [ ] Интеграционные тесты на каждый эндпоинт (CRUD)
  - [ ] Тестовая БД через переменную окружения
  - [ ] npm-скрипт `test`
...
- [ ] **4.1 Структура и базовые компоненты**
  - [ ] Настроить layout: шапка с названием приложения, основной контент
  - [ ] Настроить API-клиент (axios instance с baseURL)
  - [ ] Настроить TanStack Query provider
  - [ ] Типы TypeScript для WorkLog, WorkType (отдельный файл `types/`)
- [ ] **4.2 Таблица записей журнала**
  - [ ] Компонент `WorkLogTable`
  - [ ] Ant Design Table с колонками: дата, вид работ, объём + ед. изм., ФИО, действия
  - [ ] Сортировка по дате (встроенная в AntD Table)
  - [ ] Фильтрация по дате (DatePicker.RangePicker)
  - [ ] Кнопки «Редактировать» и «Удалить» в каждой строке
  - [ ] React Query хук `useWorkLogs(filters)`
- [ ] **4.3 Форма добавления/редактирования**
  - [ ] Компонент `WorkLogForm` (модальное окно)
  - [ ] Поля: дата, вид работ, объём, единица измерения, ФИО
  - [ ] Валидация обязательных полей (Ant Design Form + rules)
  - [ ] React Query хуки `useCreateWorkLog`, `useUpdateWorkLog`
- [ ] **4.4 Удаление записи**
  - [ ] Подтверждение через Popconfirm
  - [ ] React Query хук `useDeleteWorkLog` с инвалидацией кеша

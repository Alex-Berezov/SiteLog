...
- [x] **5.1 Тесты фронтенда** ✅
  - [x] Настроить Vitest + React Testing Library
  - [x] Тесты на рендер таблицы
  - [x] Тесты на валидацию формы
  - [x] npm-скрипт `test`
- [x] **5.2 Docker — финальная проверка** ✅
  - [x] Убедиться, что `docker-compose up` запускает всё с нуля
  - [x] Prisma миграции при старте контейнера сервера
  - [x] Сидинг при первом запуске
  - [x] Healthcheck для PostgreSQL
- [x] **5.3 README.md** ✅
  - [x] Краткое описание проекта
  - [x] Стек и обоснование выбора (2-3 предложения)
  - [x] Инструкция по запуску: `docker-compose up` + что открыть в браузере
  - [x] Инструкция по локальной разработке (без Docker)
  - [x] Структура проекта (дерево папок)
- [x] **5.4 Код-ревью и финальный линтинг** ✅
  - [x] `npm run lint` — в обоих проектах
  - [x] Убрать TODO, console.log, лишние комментарии
  - [x] Проверить, что всё собирается без ошибок
